    ...
    with tf.variable_scope(block.scope, 'block', [net]) as sc:
      for i, unit in enumerate(block.args):
        ...
        with tf.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          if output_stride is not None and current_stride == output_stride:
            net = block.unit_fn(net, rate=rate, **dict(unit, stride=1))  # 当达到设定的stride后,之后的stride都设为1
  ...
  with tf.variable_scope(
    ...
    with slim.arg_scope([slim.conv2d,
      ...
      with slim.arg_scope([slim.batch_norm], is_training=is_training):
        ...
        # net = slim.conv2d(net, 32, 3, stride=2, activation_fn=selu, scope='entry_flow/conv1_1')
        # net = slim.conv2d(net, 64, 3, stride=1, activation_fn=selu, scope='entry_flow/conv1_2')

        # net = slim.conv2d(net, 32, 3, stride=2,  scope='entry_flow/conv1_1')
        # net = slim.conv2d(net, 64, 3, stride=1,  scope='entry_flow/conv1_2')
